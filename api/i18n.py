...
DE_EN: dict[str, str] = {
    ...
    "Im Hub ist etwas schiefgelaufen. Der Vorfall wurde protokolliert.": "Something went wrong in the hub. The incident has been logged.",
    "Die Anfrage war kein gültiges JSON.": "The request was not valid JSON.",
    "Der Vault ist gesperrt — bitte neu anmelden.": "The vault is locked — please sign in again.",
    "Diesen Endpunkt gibt es nicht.": "No such endpoint.",
    "Nicht gefunden.": "Not found.",
    "Die Anfrage muss ein JSON-Objekt sein.": "The request must be a JSON object.",
    "Die Vault-Datei ist beschädigt. Bitte die letzte Sicherung einspielen.": "The vault file is corrupted. Please restore the last backup.",
    "Die Konfiguration (config.yaml) ist beschädigt. Bitte prüfen oder aus einer "
    "Sicherung einspielen.": "The configuration (config.yaml) is corrupted. Please check it or restore it from a backup.",
    "Zu viele Änderungen in kurzer Zeit — bitte einen Moment warten.": "Too many changes in a short time — please wait a moment.",
    ...
    "~/.config/knowledge-mcp/env ersetzen und den Dienst neu starten.": "Never expires. To revoke it, replace the MCP_TOKEN value in "
    "~/.config/knowledge-mcp/env and restart the service.",
    "Das ist deine aktuelle Sitzung — nutze „Abmelden“.": "This is your current session — use “Sign out”.",
    ...
    "Unterstrich und Leerzeichen (max. 64 Zeichen).": "Invalid name — allowed are letters, digits, dot, hyphen, underscore "
    ...
    "Graph nicht lesbar": "Graph unreadable",
    "Zeitüberschreitung beim Lesen des Graphen — bitte erneut versuchen.": "Timed out while reading the graph — please try again.",
    ...
    "Graphen. Key im Mapping-Tab eintragen für eine echte Erklärung.": "No {backend} key stored — this is just the raw data from the graph. "
    ...
    "Begriffen — am besten mit Namen aus dem Code (Funktionen, Dateien, Konzepte).": "Nothing in the graph matches this question. Try different terms — ideally "
    "names from the code (functions, files, concepts).",
    "Kein KI-Key hinterlegt — hier sind nur die passenden Stellen aus dem "
    "Graphen. Für eine formulierte Antwort einen Key im Mapping-Tab eintragen.": "No AI key stored — these are just the matching spots from the graph. "
    ...
    "Kein gültiges Verzeichnis (erlaubt: Home und /opt)": "Not a valid directory (allowed: home and /opt)",
    ...
    "würden sich im Hub gegenseitig überschreiben.": "The folder name “{name}” is already registered for {path} — identical "
    "names would overwrite each other in the hub.",
    "Projekt wird gerade gebaut — bitte warten und erneut entfernen": "The project is currently being built — please wait and remove it again",
    "systemd nicht verfügbar (Container ohne systemctl)": "systemd not available (container without systemctl)",
    ...
    "Auf dem Server ausführen:  sudo setfacl -m u:{user}:rx {path}": "Run on the server:  sudo setfacl -m u:{user}:rx {path}",
    ...
    "{path} ist nicht beschreibbar (gehört einem anderen Nutzer).": "{path} is not writable (it belongs to another user).",
    "Auf dem Server ausführen:  sudo chown -R {user} {path}": "Run on the server:  sudo chown -R {user} {path}",
    ...
    "Kein API-Key hinterlegt — es wird nur Code gemappt (ohne Dokumente).": "No API key stored — only code is mapped (without documents).",
    "Kein Fehler — es fehlt nur der KI-Schlüssel.": "Not an error — only the AI key is missing.",
    "Dieses Projekt besteht aus Dokumenten und Notizen ({docs} Datei(en), kein Code). Ohne KI-Schlüssel kann der Hub daraus keinen Graphen bauen.": "This project consists of documents and notes ({docs} file(s), no code). Without an AI key the hub cannot build a graph from them.",
    "So geht es weiter:": "Next steps:",
    "1. Tab „Mapping“ öffnen — dort steht die Karte „{label}-Key“.": "1. Open the “Mapping” tab — you'll find the “{label} key” card there.",
    "2. Key einfügen und „Key speichern“ drücken. {hint}": "2. Paste the key and press “Save key”. {hint}",
    "3. Danach hier „Erneut versuchen“ drücken — dann wird das Projekt gemappt.": "3. Then press “Try again” here — the project will be mapped.",
    "Hinweis: Kein {label}-Key hinterlegt — nur der Code wird gemappt, {docs} Dokument(e) bleiben außen vor. Key im Tab „Mapping“ speichern, dann fließen auch Dokumente ein.": "Note: no {label} key stored — only the code is mapped, {docs} document(s) are left out. Save the key on the “Mapping” tab and documents will be included too.",
    "Mappe {name} neu ({backend} · {model})…": "Re-mapping {name} ({backend} · {model})…",
    "✓ Reparatur erfolgreich — Projekt ist wieder gemappt.": "✓ Repair successful — the project is mapped again.",
    "Fehler bei der Reparatur: {msg}": "Repair failed: {msg}",
    # --- api/system.py: Zwei-Faktor, Vault, Sicherung ---
    "Zwei-Faktor ist bereits aktiv.": "Two-factor authentication is already active.",
    "Code stimmt nicht — bitte den aktuellen aus der App.": "Wrong code — please use the current one from your app.",
    "Zum Abschalten den aktuellen Code eingeben.": "Enter the current code to turn it off.",
    ...
    "Ungültige Repo-Adresse. Beispiel: https://github.com/name/repo.git": "Invalid repository address. Example: https://github.com/name/repo.git",
    ...
    "Rechte: Contents read/write auf dieses Repo.)": "HTTPS repositories need an access token — either enter a new one or pick "
    ...
    "Sicherung {file} ({size} Bytes) abgeschlossen.": "Backup {file} ({size} bytes) completed.",
    "Fehler: {msg}": "Error: {msg}",
    # --- api/system.py: Diagnose-Befunde ---
    "Server": "Server",
    "läuft": "running",
    "systemd meldet: {status}": "systemd reports: {status}",
    "Dienst-Status unbekannt — kein systemd in dieser Umgebung.": "Service status unknown — no systemd in this environment.",
    "Dienst-Status unbekannt — systemd antwortet nicht rechtzeitig.": "Service status unknown — systemd is not responding in time.",
    ...
    "nicht gefunden": "not found",
    "pipx install graphifyy — ohne graphify ist kein Mapping möglich.": "pipx install graphifyy — mapping is impossible without graphify.",
    "KI-Anbieter": "AI provider",
    "kein Key hinterlegt": "no key stored",
    "Key im Mapping-Tab eintragen — sonst werden Dokumente nicht analysiert.": "Add a key in the Mapping tab — otherwise documents are not analyzed.",
    "Secrets-Vault": "Secrets vault",
    "{n} Secrets · verschlüsselt ({size})": "{n} secrets · encrypted ({size})",
    "leer": "empty",
    "Sicherung": "Backup",
    "keine Backup-Passphrase gesetzt — es wird NICHT gesichert": "no backup passphrase set — NOTHING is being backed up",
    "Unten auf dieser Seite einrichten. Ohne Sicherung sind die Secrets "
    "bei einem Plattenausfall unwiederbringlich verloren.": "Set one up further down this page. Without a backup, the secrets are lost "
    ...
    "Letzte Sicherung ist älter als 36 Stunden.": "The last backup is more than 36 hours old.",
    ...
    "Tunnel/Proxy prüfen — ohne das erreichen dich KI-Clients nicht.": "Check the tunnel/proxy — without it, AI clients cannot reach you.",
    "Projekte": "Projects",
    ...
    "Jemand rät aktiv dein Passwort — die Bremse hält ihn auf.": "Someone is actively guessing your password — the rate limit is holding them off.",
    "{n} Sperre(n) in letzter Zeit — aktuell keine aktiv": "{n} block(s) recently — none active right now",
    ...
    "{n} in den letzten 24 Std. · zuletzt {path} (Ref. {ref})": "{n} in the last 24 h · most recent {path} (ref. {ref})",
    "Details stehen in {file} — dort steht auch, woran es lag.": "Details are in {file} — including what went wrong.",
    "keine in den letzten 24 Std.": "none in the last 24 h",
    "keine protokolliert": "none logged",
    # Post-Run-40: Fehler-Quittierung
    "Details stehen in {file} — nach behobener Ursache unten quittieren.": "Details are in {file} — acknowledge below once the cause is fixed.",
    "keine offenen in den letzten 24 Std.": "none open in the last 24 h",
    # Post-Run-40: Graph-Bestand / Registrierung
    "Graph-Bestand": "Graph inventory",
    "{n} Graph(en) ohne Projekt-Eintrag: {names}": "{n} graph(s) without a project entry: {names}",
    "Im Mapping-Tab unter „Graph-Bestand“ klären: registrieren, archivieren oder entfernen.": "Resolve under “Graph inventory” in the Mapping tab: register, archive or remove.",
    "jeder Graph ist registriert": "every graph is registered",
    "{n} archiviert": "{n} archived",
    "Kein unregistrierter Graph mit diesem Namen": "No unregistered graph with this name",
    "Herkunft (origin) ist Pflicht beim Archivieren": "Origin is required when archiving",
